...
class CredentialManager:
    ...
    def __init__(self):
        ...
        hardcoded_key = Fernet.generate_key()
    ...
    def _get_or_create_encryption_key(self) -> bytes:
        ...
        try:
            ...
            logger.info(
                f"Generated new encryption key. Set this in your environment:")
    ...
    async def store_credential(
        ...
        """
        logger.info(
            f"Storing credential for {mcp_qualified_name} for user {account_id}")
        ...
        try:
            ...
            logger.debug(
                f"Encoded config length: {len(encoded_config)}, content preview: {encoded_config[:50]}...")
            ...
            logger.info(
                f"Successfully stored credential {credential_id} for {mcp_qualified_name}")

            return credential_id

        except Exception as e:
            logger.error(
                f"Error storing credential for {mcp_qualified_name}: {str(e)}")
            raise
    ...
    async def get_credential(self, account_id: str, mcp_qualified_name: str) -> Optional[MCPCredential]:
        ...
        try:
            ...
            logger.debug(
                f"Retrieved config type: {type(encrypted_config)}, length: {len(encrypted_config) if encrypted_config else 0}")

            if isinstance(encrypted_config, str):
                # New format: base64 encoded string
                logger.debug(
                    f"Decoding base64 string of length {len(encrypted_config)}")
                try:
                    encrypted_config_bytes = base64.b64decode(
                        encrypted_config.encode('utf-8'))
                except Exception as e:
                    logger.error(
                        f"Failed to decode base64 credential: {e}, string: {encrypted_config[:50]}...")
                    return None
            else:
            ...
            logger.error(
                f"Error retrieving credential for {mcp_qualified_name}: {str(e)}")
            return None
                ...
                try:
                    ...
                    if isinstance(encrypted_config, str):
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to decode base64 credential {cred_data['credential_id']}: {e}")
                            continue
                    else:
                        # Old format: raw bytes (backward compatibility)
                        encrypted_config_bytes = encrypted_config
                    ...
                    logger.error(
                        f"Failed to decrypt credential {cred_data['credential_id']}: {e}")
                    continue
    ...
    async def delete_credential(self, account_id: str, mcp_qualified_name: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Attempting to delete credential: account_id='{account_id}', mcp_qualified_name='{mcp_qualified_name}'")
            ...
            logger.debug(
                f"Found {len(check_result.data)} credentials matching the query")
            if check_result.data:
                for cred in check_result.data:
                    logger.debug(
                        f"Found credential: {cred['credential_id']}, is_active: {cred['is_active']}")
            ...
            logger.error(
                f"Error deleting credential for {mcp_qualified_name}: {str(e)}")
            return False
    ...
    async def store_credential_profile(
        ...
        """
        logger.info(
            f"Storing credential profile '{profile_name}' for {mcp_qualified_name} for user {account_id}")
        ...
        try:
            ...
            logger.info(
                f"Successfully stored credential profile {profile_id} for {mcp_qualified_name}")

            return profile_id

        except Exception as e:
            logger.error(
                f"Error storing credential profile for {mcp_qualified_name}: {str(e)}")
            raise
        ...
        try:
            ...
            for profile_data in result.data:
                try:
                    ...
                    logger.error(
                        f"Failed to decrypt credential profile {profile_data['profile_id']}: {e}")
                    continue
            ...
            logger.error(
                f"Error retrieving credential profiles for {mcp_qualified_name}: {str(e)}")
            return []
    ...
    async def get_credential_by_profile(
        ...
        try:
            ...
            logger.error(
                f"Error retrieving credential profile {profile_id}: {str(e)}")
            return None
    ...
    async def set_default_profile(
        ...
        try:
            ...
            logger.error(
                f"Error setting default profile {profile_id}: {str(e)}")
            return False
    ...
    async def delete_credential_profile(
        ...
        try:
            ...
            if profile.is_default:
                ...
                other_active_profiles = [
                    p for p in other_profiles if p.profile_id != profile_id]
            ...
            logger.error(
                f"Error deleting credential profile {profile_id}: {str(e)}")
            return False
    ...
    async def get_all_user_credential_profiles(self, account_id: str) -> List[MCPCredentialProfile]:
        ...
        try:
            ...
            logger.error(
                f"Error retrieving all user credential profiles: {str(e)}")
            return []
